 ...
 */
export async function selectColivingForSubscriber(subscriber: SubscriberData): Promise<string | null> {
  const db = admin.firestore();
  ...
  try {
    ...
    let colivingsSnapshot: FirebaseFirestore.QuerySnapshot;
    ...
    try {
      ...
      if (colivingsSnapshot.empty) {
        ...
        for (const variation of variations) {
          ...
          if (!colivingsSnapshot.empty) break;
        }
      }
...
export async function generatePersonalizedRecommendation(
  ...
  colivingId: string
): Promise<ColivingRecommendation | null> {
  const db = admin.firestore();
  ...
  try {
    ...
    let nearbyPlaces: Array<{ name: string; category: string; rating?: number; distance?: string }> = [];
      ...
      if (nearbyData?.nearby_places) {
        ...
        priorityCategories.forEach(category => {
          ...
          if (Array.isArray(categoryPlaces) && categoryPlaces.length > 0) {
            ...
            categoryPlaces
              .filter((place: any) => place.name && place.rating && place.rating >= 4.0)
              .slice(0, 2)
              .forEach((place: any) => {
    ...
    let communityLinks: Array<{ platform: string; name: string; link: string; memberCount?: number }> = [];
 ...
 */
export function generatePersonalizedEmailHTML(
  subscriber: SubscriberData, 
 ...
 */
export function generatePersonalizedEmailText(
  subscriber: SubscriberData, 
  recommendation: ColivingRecommendation
): string {
  const { coliving, nearbyPlaces, communityLinks } = recommendation;
...
`;
  ...
  if (nearbyPlaces.length > 0) {
    ...
    nearbyPlaces.forEach(place => {
      ...
      if (place.rating) text += ` ⭐ ${place.rating}/5`;
      text += `\n`;
    });
    text += `\n`;
  }
 ...
 */
export async function sendPersonalizedColivingEmail(
  subscriber: SubscriberData,
  recommendation: ColivingRecommendation
  ...
  try {
    ...
    console.log(`✅ Email sent successfully to ${subscriber.email} - Message ID: ${result.data?.id}`);
 ...
 */
export async function sendBatchPersonalizedEmails(
  emailData: Array<{ subscriber: SubscriberData; recommendation: ColivingRecommendation }>
): Promise<{
  totalSent: number;
  totalFailed: number;
...
export async function sendTestEmail(
  ...
  try {
    ...
    console.log(`✅ Test email sent successfully to ${testEmail} - Message ID: ${result.data?.id}`);
    return { success: true, messageId: result.data?.id };
