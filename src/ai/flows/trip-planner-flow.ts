...
const ColivingSuggestionSchema = z.object({
  ...
  reason: z.string().describe('A brief reason why this coliving space is recommended for the user.'),
});

const ActivitySuggestionSchema = z.object({
    name: z.string().describe('Name of the cafe or restaurant.'),
    reason: z.string().optional().describe('Brief reason for recommendation (e.g., good wifi, quiet, local cuisine).'),
...
const DailyItineraryItemSchema = z.object({
  ...
  morningActivity: z.string().describe('Suggested morning activity, work block, or cafe visit.'),
  afternoonActivity: z.string().describe('Suggested afternoon activity, work block, or exploration.'),
  eveningActivity: z.string().describe('Suggested evening activity, e.g., dinner, social event, relaxation.'),
});

const TripPlanOutputSchema = z.object({
  destinationOverview: z.string().describe('A brief overview of the recommended destination and why it fits the user\'s preferences.'),
  colivingSuggestion: ColivingSuggestionSchema.describe('A single, top coliving space recommendation.'),
  dailyItinerary: z.array(DailyItineraryItemSchema).length(3).describe('A sample 3-day itinerary, detailing morning, afternoon, and evening plans.'),
  cafeSuggestions: z.array(ActivitySuggestionSchema).length(2).describe('Two cafe suggestions suitable for working, including name and reason.'),
  restaurantSuggestions: z.array(ActivitySuggestionSchema).length(2).describe('Two restaurant suggestions, including name, cuisine, and reason.'),
});
...
1.  Destination Overview: A brief (2-3 sentences) overview of why the chosen location is suitable based on the user's inputs.
2.  Coliving Suggestion: Recommend ONE coliving space. Provide its name, a plausible fictional address, and a short reason it fits the user.
3.  Daily Itinerary: Provide a sample 3-DAY itinerary. For each day, suggest distinct morning, afternoon, and evening activities. These activities can include focused work blocks, suggestions for working at a cafe, local exploration, cultural experiences, or dining.
4.  Cafe Suggestions: Recommend TWO cafes suitable for working. Provide their names and a brief reason for the recommendation (e.g., good Wi-Fi, quiet atmosphere, good coffee).
5.  Restaurant Suggestions: Recommend TWO restaurants. Provide their names, type of cuisine, and a brief reason for the recommendation (e.g., local specialty, good reviews, ambiance).

Focus on providing concrete and diverse suggestions that align with a digital nomad lifestyle. Ensure all output fields are populated according to the schema.`,
});
